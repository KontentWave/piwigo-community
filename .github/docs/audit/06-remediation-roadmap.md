...
1. Replace `community_switch_user_to_admin` with Community-owned, method-specific adapters.
   Status 2026-07-31: partially completed. `pwg.images.addSimple`, `pwg.images.upload`, `pwg.images.uploadAsync`, `pwg.images.add`, `pwg.images.addChunk`, and `pwg.images.uploadCompleted` now use Community-owned wrappers with normalized authorization and object-mutation checks. Both upload-completion public names share one scoped service without status elevation or global lounge draining. `pwg.images.uploadAsync` and the legacy `pwg.images.add` + `pwg.images.addChunk` lifecycle persist plugin-owned, lock-protected upload state across independent requests. `pwg.images.upload` rejects Community `update_mode` because the core delegate cannot safely pin the replacement target. Ambient elevation remains for `pwg.tags.add`, `pwg.images.exist`, `pwg.images.checkUpload`, `pwg.images.checkFiles`, and `pwg.session.getStatus`.
