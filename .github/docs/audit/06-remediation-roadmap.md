...
1. Replace `community_switch_user_to_admin` with Community-owned, method-specific adapters.
   Status 2026-07-31: partially completed for SEC-01 Slice 1 only. `pwg.images.addSimple`, `pwg.images.upload`, and `pwg.images.uploadAsync` now use Community-owned wrappers with normalized destination authorization and object-mutation checks; `pwg.images.uploadAsync` also persists a plugin-owned, lock-protected manifest and chunk store across independent requests so chunk writes no longer depend on ambient core buffering. `pwg.images.upload` currently rejects Community `update_mode` because the core delegate cannot safely pin the authorized replacement target, while the remaining elevated upload/helper methods still need method-specific adapters.
...
5. Add ownership/category checks to upload completion, image replacement, format attachment, update mode, and other image mutation.
   Status 2026-07-31: partially completed for SEC-01 Slice 1 only. `pwg.images.addSimple` and `pwg.images.uploadAsync` replacement now require ownership and, for guest/generic users, current-session provenance immediately before delegation, with `pwg.images.uploadAsync` revalidating ownership/session provenance again on the completion request immediately before final persistence; `pwg.images.upload` now rejects unauthorized `format_of` and rejects Community `update_mode` entirely until replacement targets can be pinned before delegation.
