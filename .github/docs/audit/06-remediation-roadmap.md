...
1. Replace `community_switch_user_to_admin` with Community-owned, method-specific adapters.
   Status 2026-07-31: partially completed for SEC-01 Slice 1 only. `pwg.images.addSimple`, `pwg.images.upload`, `pwg.images.uploadAsync`, `pwg.images.add`, and `pwg.images.addChunk` now use Community-owned wrappers with normalized destination authorization and object-mutation checks. `pwg.images.uploadAsync` and the legacy `pwg.images.add` + `pwg.images.addChunk` lifecycle now persist plugin-owned, lock-protected upload state across independent requests so chunk writes no longer depend on ambient core buffering. `pwg.images.upload` currently rejects Community `update_mode` because the core delegate cannot safely pin the authorized replacement target, while the remaining elevated helper methods still need method-specific adapters.
...
5. Add ownership/category checks to upload completion, image replacement, format attachment, update mode, and other image mutation.
   Status 2026-07-31: partially completed for SEC-01 Slice 1 only. `pwg.images.addSimple`, `pwg.images.uploadAsync`, and the legacy `pwg.images.add` finalizer replacement path now require ownership and, for guest/generic users, current-session provenance immediately before delegation or final persistence; `pwg.images.uploadAsync` revalidates ownership/session provenance again on the completion request immediately before final persistence; `pwg.images.upload` now rejects unauthorized `format_of` and rejects Community `update_mode` entirely until replacement targets can be pinned before delegation.
