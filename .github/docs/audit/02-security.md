...
**Residual risk after Slice 1:** The broader SEC-01 finding remains open for the other upload transports and admin-only helper methods that still rely on ambient elevation or lack equivalent object and destination authorization, especially `pwg.images.add`, `pwg.images.addChunk`, chunk/check helpers, and upload-completion helpers. The new `uploadAsync` state is file-backed rather than transactional database state, so it closes the manifest-binding gap without claiming the broader quota-reservation or helper-surface work described in SEC-04 and the remaining SEC-01 items.
