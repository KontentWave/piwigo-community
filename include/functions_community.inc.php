...
function community_get_user_permissions($user_id)
{
  ...
  {
    $cached_permissions = $_SESSION['community_user_permissions'];

    if (
      !empty($conf['community']['user_albums'])
      and !is_admin()
      and !empty($cached_permissions['user_album'])
    )
    {
      $user_album_category_id = community_get_user_album($user_id);
      $cached_create_categories = array_map('intval', $cached_permissions['create_categories'] ?? array());

      if (!empty($user_album_category_id) and in_array((int) $user_album_category_id, $cached_create_categories, true))
      {
...
;';
  ...
  elseif (count($return['create_categories']) > 0)
  {
    ...
    $return['create_categories'] = array_unique(
      array_merge(
        array_map('intval', $return['create_categories']),
        get_subcat_ids($return['create_categories'])
      )
    );
  ...
  if ($return['user_album'])
  {
    ...
    if (!empty($user_album_category_id) and !is_admin())
    {
      ...
      $return['create_whole_gallery'] = false;
      $return['upload_categories'] = array_values(array_unique(array_intersect($user_album_descendants, array_map('intval', $return['upload_categories']))));
      $return['create_categories'] = array_values(array_unique(array_intersect($user_album_tree, array_map('intval', $return['create_categories']))));
    }
  }
