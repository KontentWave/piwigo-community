...
function community_require_two_factor_for_album_management()
{
  global $user;
  ...
  if (!function_exists('tf_bootstrap_profile_liveness_guard_helpers'))
  {
    $two_factor_functions = PHPWG_ROOT_PATH . 'plugins/two_factor/includes/functions.inc.php';
    if (defined('TF_REALPATH') && file_exists($two_factor_functions))
    {
      include_once($two_factor_functions);
    }
  }

  if (!function_exists('tf_is_profile_liveness_guard_eligible_user')
    || !function_exists('tf_user_has_enabled_two_factor')
    || !function_exists('tf_get_profile_url'))
  {
    return;
  }

  $user_id = (int) $user['id'];
  if (!tf_is_profile_liveness_guard_eligible_user($user_id))
  {
    return;
  }

  if (!tf_user_has_enabled_two_factor($user_id))
  {
    if (!defined('IN_WS'))
    {
...
function community_ws_session_getStatus($params, &$service)
{
  ...
  else
  {
    ...
    $res['create_categories'] = $user['community_permissions']['create_categories'];
  }
