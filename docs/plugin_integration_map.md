...
| Legacy chunk upload  | `pwg.images.addChunk`, `pwg.images.add`, `add_uploaded_file()`                                | `original_sum` names buffer chunks and is reused as an image checksum; legacy registration has no checksum-shape constraint. | Caller-controlled checksum reaches regular expressions, paths, and unescaped SQL in core and Community.                                                            |
...
- Validate `original_sum` as exactly 32 hexadecimal characters before filesystem or database use; escaping remains mandatory at each sink.
